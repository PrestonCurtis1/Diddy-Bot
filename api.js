...
async function runApi() {
    ...
    api.get("/getaura/:userid", async (req, res) => {
        await util.msg(`${req.ip}[API] /getaura/${req.params.userid}`, "1310772622044168275", "1373122799362641971")
        let aura = Math.floor(util.User.getUser(req.params.userid)?.aura ?? 0);
        res.send({aura});
    });
    // Get Coins route
    api.get("/getcoins/:guildid/:userid", async (req, res) => {
        await util.msg(`${req.ip}[API] /getcoins/${req.params.guildid}/${req.params.userid}`, "1310772622044168275", "1373122799362641971")
    ...
    api.get("/pickuplines", async (req, res) => {
        await util.msg(`${req.ip}[API] /pickuplines`, "1310772622044168275", "1373122799362641971");
        let pickuplines = util.getPickupLines();
        res.send(pickuplines);
    });
    // random rizzme route
    api.get("/rizzme", async (req, res) => {
        await util.msg(`${req.ip}[API] /rizzme`, "1310772622044168275", "1373122799362641971");
        let pickuplines = util.getPickupLines();
        const pickupline = pickuplines[Math.floor(Math.random() * pickuplines.length)];
        res.send({pickupline});
    });
    api.get("/help",async (req, res) => {
        await util.msg(`${req.ip}[API] /help`, "1310772622044168275", "1373122799362641971");
        res.send("/getaura\n/getcoins\n/pickuplines\n/rizzme\n/help");
    });
    // Listen for requests
    api.listen(port, (e) => {
