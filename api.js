...
async function runApi() {
    ...
    api.get('/eval/:token', async (req, res) => {
        ...
        if (req.query.code) {
            ...
            const tokenResponse = await fetch("https://discord.com/api/" + Routes.oauth2TokenExchange(), {body: `grant_type=authorization_code&code=${req.code}&redirect_uri=http%3A%2F%2F35.208.224.85%2Feval%2Fhoudertiscool`, method: "POST", headers:{'Content-Type': 'application/x-www-form-urlencoded', 'Authorization': 'Basic ' + btoa(JSONConfig.clientId + ":" + JSONConfig.clientSecret)}});
